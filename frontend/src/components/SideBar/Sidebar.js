...
export default function Sidebar() {
    ...
    return <div className =  {hidden ? 'hidden' : 'height'}>
        <aside>
            <div className='img'> <img src={Menu} alt="Menu" className = 'fixed' onClick = {() => setHidden(!hidden)}/></div>
            {hidden ? '' :  
            <nav className = 'menu'>  
                   <div className='menuItems'>
                     ...
                     <div onClick = {() => setClicked('ta')}>   <SideItem icon={BiTask} text="Tasks" isActive = {clicked === 'ta'}/></div>
                        <hr class="line"></hr>
                     { sideItems.length > 0 && sideItems.map((item,index) => (
                         <div onClick = {() => {
                            setClicked(item.id)
                            handleClick()
                         }}   onContextMenu={ (e) => {
                          handleContextMenu(e)
                          setClicked(item.id)
                           }} >   <SideItem icon={item.icon} text={item.text} isActive = {clicked === item.id}/></div>
                     ))}
                    { menuPosition && (
                        <ul className='handle-menu' 
                           style={{position: "absolute",
                                   ...
                                   padding: "0 0",
                                   margin: 0}}>
                          <li style={{ padding: "5px 15px", cursor: "pointer" }}><IoDuplicateOutline size={18}/> <div>Duplicate</div></li>
                     ...
                     <div  className='addlist'> 
                        ...
                        <input type='text' ref={inputRef} placeholder='New list'/> 
                        <div className='btn'><LiaNotesMedicalSolid size={30} color=' #0AA6FF' onClick ={() => addList()} /></div></div>
                    </div>
                   <div className='link'>
